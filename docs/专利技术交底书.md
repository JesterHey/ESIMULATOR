# 基于数据流图（DFG）的数字电路线性特性分析与可视化系统 — 技术交底书
...
数字电路的线性/非线性性质（例如在 GF(2) 或算术域下）直接影响到电路的可综合性、可验证性、可攻击性分析（如差分/线性密码分析）、以及后续优化（如融合、分区、综合策略）。现有方法多从门级网表或布尔函数层面分析，缺少一种以数据流图（DFG）绑定级粒度、包含源/目的代码行信息的全面分析与可视化方案，难以在不降级为门级复杂度的情况下快速给出“可视化可解释”的线性标注结果。
...
1) DFG 生成：对目标模块 m 与源文件 v 运行 Pyverilog 示例脚本，得到 `dfg_files/<m>_dfg.txt`。
...
3) GF(2) 与算术域（arith）差异：
   - 算法支持以“模式”确定线性运算符集合。例如：在 arith 模式中可将 XOR 排除在线性集之外，而在 gf2 模式中将 XOR 视为线性；移位与串接在两种模式下均视为非线性。
4) Concat 特别规则：Concat 被显式标记为非线性（intrinsic_linear=0），即使子式线性，整体仍非线性。
...
1) 线性图 JSON（`*_linearity_graph.json`，要点）：

- nodes: [{ id, label, is_linear, intrinsic_linear, line_no_src, line_no_dst, ... }]
- edges: [{ src, dst, type, fusable?, ... }]

1) Bind 掩码 JSON（`*_bind_masks.json`，要点）：

- binds: [{ name, is_linear, reason: ["nonlinear_op", "branch", ...], mask, line_no, ... }]

1) 分析汇总 JSON（`linearity_analysis.json`，要点）：

- summary: { total_expressions, linear_count, nonlinear_count, breakdown_by_reason }
- samples: [...可选...]

1) 信号对比 JSON（`*_signal_compare.json`，要点）：

- summary: { total_names, verilog_signals, bind_count, coverage, ... }

注：字段以当前实现为准，以上为常见/推荐字段；系统实际输出随版本演进而扩展。
...
说明：为便于版式统一，所有插图已集中放置在文档末尾“附图”一节，此处仅给出说明。
...
### 8.2 实施例二（instruction_decode 模块）
...
- 一种存储介质，存储有用于执行上述方法步骤的指令。

## 10. 应用场景

- 硬件安全性分析与侧信道研究中的线性/非线性特性评估。
...
注：若 PNG 尚未生成，请先执行批处理脚本并带上 `--visualize --render-png` 参数，或对对应 DOT 使用 Graphviz 转换。
