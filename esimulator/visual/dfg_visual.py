...
def classify_expression(tree: str):
    reasons: Set[str] = set()
    if 'Branch' in tree:
        reasons.add('Branch')
...
def build_graph_data(signals: Dict[str, List[str]], binds: Dict[str,str]):
    nodes: Dict[str, Dict] = {}
...
def write_dot(nodes: Dict[str, Dict], edges: List[Tuple[str,str]], out_path: str):
    ...
    for name, info in nodes.items():
        ...
        if is_lin is True: color=COLOR_LINEAR; status='L'
        elif is_lin is False: color=COLOR_NONLINEAR; status='NL'
        else: color=COLOR_UNKNOWN; status='?'
...
def write_interactive_html(nodes: Dict[str, Dict], edges: List[Tuple[str,str]], metrics: Dict, out_path: str):
    ...
    for idx,(name,info) in enumerate(nodes.items()):
        index_map[name]=idx
        node_list.append({'id':name,'linear':info['is_linear'],'reasons':info['reasons'],'types':info['types']})
    link_list=[]
    for s,d in edges:
        if s in index_map and d in index_map: link_list.append({'source':s,'target':d})
...
def visualize_from_dfg(dfg_path: str, out_dir: str, stem: Optional[str]=None, *, focus: Optional[str]=None, depth: int=2, keep: Optional[str]=None, html: bool=True, dot: bool=True):
    signals, binds = parse_dfg(dfg_path)
    nodes, edges = build_graph_data(signals, binds)
    ...
    if dot:
        write_dot(nodes, edges, os.path.join(out_dir, f"{stem}.dot"))
    if html:
